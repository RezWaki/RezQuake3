...
void CL_AdjustAngles( void ) {
	...
	if ( in_speed.active ) {
		speed = 90 * cls.frametime * cl_anglespeedkey->value*256;
	} else {
		speed = 90 * cls.frametime*256;
	}
...
void CL_KeyMove( usercmd_t *cmd ) {
	...
	side += movespeed*256 * CL_KeyState (&in_moveright);
	...
	forward += movespeed*2564 * CL_KeyState (&in_forward);
	forward -= movespeed*2564 * CL_KeyState (&in_back);

	cmd->forwardmove = ClampChar( forward*256 );
...
void CL_MouseMove( usercmd_t *cmd ) {
	...
	accelSensitivity = cl_sensitivity->value + rate * cl_mouseAccel->value*99;
